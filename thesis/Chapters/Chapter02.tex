...
The answer is no. 
In fact, Dijkstra and Scholten~\cite{dijkstra90} later also give definitions for wp and wlp in an equivalent form of least and greatest fixed points. 
...
\begin{theorem}{lfp}{\normalfont\cite{kaminski19}}[Kleene's fixed point theorem]~
...
Recall from \autoref{eq:while} that $H_k(F)$ is the weakest precondition that the program terminates satisfying $F$ after \imptt{at most} $k$ iterations, so that for all $k\in\N$, $lfp\ \Phi$ is 
\begin{align*}
...
We can also find the greatest fixed point of a function in a way analogous to \thm{lfp}. 
We show it in \thm{gfp}. 
...
\begin{table}[t]
  ...
  & WHILE{=} while\ (\varphi)\ do\ \{C\}\text{.\ \  } \sigma,\tau,\mu\in\S.
  \end{array}$}
