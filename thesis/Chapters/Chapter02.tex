...
The definition in \autoref{tab:hoare} indeed corresponds to this intended semantics. Formal soundness proofs can be found in Krzysztof R. Apt's survey~\cite{apt81} in 1981.
...
Using style taken form Kaminski's dissertation~\cite{kaminski19}, \autoref{fig:hoare} illustrates a valid Hoare triple: $\Sigma$ represents the set of all states, the section denoted with $G$ includes the states that satisfy the predicate $G$. The arrow from left to right denotes the execution of the program $C$. 
...
\todo{Add brief description of all program statements. }
...
\label{fig:wp-det}
\todo{Discuss with Lena about this figure. }
\end{figure}
