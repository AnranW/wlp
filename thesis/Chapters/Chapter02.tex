...
We can see that on the right side, the arrows from $G$ to non-termination and from $\neg G$ to $F$ are absent. 
...
\end{equation}
% Replacing the existential quantifier by disjunctions yields: 
%  % \[wp.WHILE.F=(\exists k\geq 0: H_k(F))  \] \label{eq:while}
% \begin{equation}
% \hspace{2.8cm} wp.WHILE.F\equiv\bigvee_{k\geq 0} H_k(F) \equiv \bigvee_{k\geq 0} wp.IF.H_{k-1}(F) \vee H_0(F))  \label{eq:while-disj}
% \end{equation}



We state that the definition in \autoref{tab:wp-det} coincides with this definition. 
Without going too deep into domain theory, we only use one of its theorem that yields a computation for least fixed points, provided they exist. 
...
\vspace{-0.25cm}\paragraph{Step case: } 
  \begin{align*} 
    H_{k+1}(F)     & = wp.IF.H_{k}(F) \vee H_0(F) \\
                  ...
                  &\hspace{0.45\textwidth} | \ \text{induction hypothesis; predicate calculus} \\
...
Here we assume a setting with \define{angelic non-determinism}, where we think that whenever non-determinism occurs, it will resolve in our favor.
...
This is a design choice that is different from Dijkstra's~\cite{dijkstra75}, where the wp for non-deterministic choice would be a conjunction, hinting at a demonic setting. 
...
The left side of \autoref{fig:wp-wlp-together} shows $wp$ with non-deterministic programs. 
...
\begin{figure}[ht!]\centering
\includegraphics[width=\textwidth]{image/wp-wlp-together.eps}
\caption{Weakest Precondition (Angelic Non-determinism), Weakest Liberal Precondition (Demonic Non-determinism)}
...
While wp-transformer excludes non-termination, wlp-transformer takes a more liberal approach. 
The weakest precondition delivers a precondition so that the program terminates and the postcondition is \imptt{reachable}. 
...
The postcondition in the wlp setting is not only reachable, but also guaranteed upon termination, because we regard the non-deterministic choice as demonic, again favoring to establish Galois connection~\cite{zhang22}. 

We define the weakest liberal precondition transformer in \autoref{tab:wlp}. 
A graphical representation can be found in the right side of \autoref{fig:wp-wlp-together}. 
...
\begin{table}[ht!]\centering
    ...
    \hline\hline
      ...
      {\color{Maroon}$\{C_1\}\square \{C_2\}$} & {\color{Maroon}$wlp.C_1.F\wedge wlp.C_2.F$}\\
      {\color{Maroon}$while\ (\varphi)\ \{C'\}$}& {\color{Maroon} $gfp\ X.(\neg\varphi\wedge F)\vee(\varphi\wedge wlp.C'.X)$}\\
      \hline\hline
    \end{tabular}
    \caption{The Weakest Liberal Precondition Transformer}
    \label{tab:wlp}
\end{table}
\todo{Explain the use of gfp. Possibly relate to page 185 of Dijkstra and Scholten 1990 book. }
...
% \caption{Weakest Liberal Precondition (Demonic Non-determinism)}
...
% \[wlp.C.false = preconditions that lead to nontermination\]
