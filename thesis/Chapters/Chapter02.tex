...
Secondly, it is standard practice to require the semantics of $WHILE$ to be equivalent to the semantics of $if(\varphi)\{C';WHILE\}\ else\ \{skip\}$
...
To express the meaning of programs, we choose \define{big-step semantics} to describe executions of a program. 
...
Here we overload the symbol for function application \mathl{.} so that it applies to \define{expressions} as well, but without specifying the set of expressions hence restricting our programming language. 
...
In our big-step semantics, non-termination of $C$ with initial state $\sigma$ is denoted by the nonexistence of state $\tau\in\S$ such that $\sigma\goto{C}\tau$. 
