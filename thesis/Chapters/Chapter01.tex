...
\begin{enumerate}
    \item We proved that the definition of the weakest precondition transformer of while-programs by Dijkstra~\cite{dijkstra75} and the one using the least fixed point coincide, and gave intuition to explain the necessity of using the least fixed point instead of using any other fixed point. 
    ...
    We then present the results as a graph like in \autoref{fig:intro-wp-sp-galois}.
    \begin{figure}[!h]
        \centering
    ...
    \item We also notice a special group of preconditions, under whose control the execution is possible to terminate in both the desired postcondition and its opposite, like the green parts in \autoref{fig:intro-g}. 
    \begin{figure}[!h]
        \centering
